...
app.controller("ParametrageCtrl",function($location,$scope, $ionicModal, $cordovaSQLite, $http, ionicToast, $state, $ionicPlatform, $fileFactory,$timeout, $ionicLoading, $ionicPopup,$rootScope, $q,$ionicHistory,$state,$window, $localStorage, ConnectivityMonitor){
               ...
               //         //  console.info(res);
               //              // if(res.rows.length==0){
               //                     console.info("------------------------------ "+cid+", "+id+", "+ id_com+", "+JSON.stringify(support)+" , "+id_projet);
               ...
               function addSupportToDB(i, id_com, id_projet){
                       var deferred = $q.defer();
                       var data = $scope.supportsCom[i];
                       $scope.cmptSupp++;
                       var cid  =data.cid;
                       var id  = data.id;
                       var support={};
                       ...
                       console.info("------------------------------ "+cid+", "+id+", "+ id_com+", "+JSON.stringify(support)+" , "+id_projet);
                        var sqlInsertSupport= "INSERT INTO supports (cid, id, id_projet, id_commune, support, userId) VALUES(?,?,?,?,?,?)";
                       $cordovaSQLite.execute(db, sqlInsertSupport,[cid, id , id_projet, id_com, JSON.stringify(support), $scope.userId]).then(function(res) {
                            console.info(res);
                            deferred.resolve();
                       });
               ...
               function loadSupportsData(id_com, res, id_projet){
                       ...
                       for(var i = 0; i<res.length; i++){
                             ...
                             chain = chain.then(function() {
                                   return  addSupportToDB( $scope.cmptSupp, id_commune, id_projet);
                             });                        
                       }
               ...
               function loadCommuneSupportServer(i){
                       ...
                       var selectedCom =  $scope.communes[$scope.countCom].id;
                       console.info(selectedCom);
                       var deferred = $q.defer();
                       $scope.countCom++;
                       ...
                       $http.get(urlDecoupage, {
                              ...
                              console.info(res.data.supports);
                              if( (res.data.supports!=undefined) && (res.data.supports!={}) ) {
                                      console.info(res.data.supports.length);
                                      if(res.data.supports.length>0){

                                            $scope.supportsCom = res.data.supports;

                                             var sqlDeleteSupports = "DELETE FROM supports WHERE id_projet='"+$scope.selectedProject+"' AND id_commune='"+selectedCom+"' AND userId="+$scope.userId;
                                             console.info(sqlDeleteSupports);
                                             $cordovaSQLite.execute(db, sqlDeleteSupports).then(function(result) {
                                                  console.info(result);
                                                  loadSupportsData(id_com, res.data.supports, $scope.selectedProject).then(function() {
                                                             console.info("woow loadSupportsData");
                                                             $scope.countCom++;
                ...
                /////////////////////// Décommenter pour utilisation tablette //////////////////////
                       ...
                       //          fs.getEntries("file:///storage/emulated/0/SayGIS/").then(function(result) {
                       //            // alert(result);
                       //              $scope.files = result;
                       //             }, function(error) {
...
app.controller("menuCtrl",function($location,$scope, $cordovaSQLite, $http, ionicToast, $state, $ionicPlatform, nonGeomtricData, $fileFactory,$timeout, $ionicLoading, $ionicPopup,$rootScope, $q, ionicToast,$ionicHistory,$state,$window, $localStorage, $ionicModal, ConnectivityMonitor){
                       ...
                       loadProjectToSynchronize().then(function(){
                            ...
                            //------------------------------------------------------------------------------------------------------//
                             $ionicPopup.show({
                                    ...
                                    buttons: [
                                      {text: 'Retour', 
                                        onTap: function(e) { return e;  $scope.projets=[]; }
                                      },
                                      {
               ...
               function dynamicDataToServer(i){
                       ...
                       $http({
                          ...
                          if(res.status==200){
                                ...
                                setElementSyncById($scope.idDynamicDataToFlag[i], res.data.id_collecte).then(function(){
                                    deferred.resolve();
                                });
                          }else{ //erreur
               ...
               function SynchronizeDynamicData(id_projet){
                ...
                //-------------------------    Re-Synchornisation  ----------------------//
                 ...
                 Re_loadProjectToSynchronize=function(){
                       var deferred = $q.defer();
                       $scope.selectedProjectToSync =null;
                       var querySelectedProject = " SELECT DISTINCT _id, name FROM projets WHERE _id IN (SELECT DISTINCT id_projet FROM dynamicData WHERE sync=1 AND userId="+$scope.userId+" )";
                             $cordovaSQLite.execute(db, querySelectedProject).then(function(res) {
                                    console.info("nbr projet à synchroniser: "+res.rows.length);
                                    if(res.rows.length > 0) {
                                     $scope.projets=[];
                 ...
                 // 1
                 $scope.Re_PopupSynchronize = function() {
                       ...
                       Re_loadProjectToSynchronize().then(function(){
                            console.info($scope.projets);
                            console.info($scope.projets.checkd);
                            if($scope.projets.length==0){
                                   ionicToast.show(' Aucune données à synchroniser ', 'middle', false, 2500);
                              ...
                              .then(function(resul) {
                                    ...
                                    if (resul){
                                             ...
                                             if(!ConnectivityMonitor.isOnline()) {
                                                    ...
                                                    Re_SynchronizeDynamicData($scope.selectedProjectToSync).then(function(){
                                                           $timeout(function () {
                                                                   $scope.selectedProjectToSync = null;
                                                                   $scope.projectSelectedToSync = false;
                                                           }, 1000);
                 ...
                 // 4
                 function Re_initializeDataObjectToSync(id_projet){
                       var deferred = $q.defer();
                        // var sqlForm= "SELECT  id, id_projet, id_exp, superficie, lat, lng, geo, data FROM dynamicData WHERE id_projet='"+id_projet+"' AND sync = 0";
                        var sqlForm= "SELECT  id, id_projet, numero, userId, superficie, exploitation, data, id_region, id_province, id_commune, geo FROM dynamicData WHERE id_projet='"+id_projet+"' AND sync = 1  AND userId="+$scope.userId;
                        console.log(sqlForm);
                 ...
                 // 7
                 function Re_dynamicDataToServer(i){
                       var data = $scope.dataToSync[i];
                       console.info("i= "+i);
                       // console.info(data);
                       // console.info(JSON.stringify(data));
                 ...
                 // 6
                 function Re_setElementSyncById(id, id_collecte){
                 ...
                 // 3
                 function Re_SynchronizeDynamicData(id_projet){
                       var deferred = $q.defer();
                       $scope.dataToSync = [];
                       $scope.Recount =0;
                        var chainage = $q.when();
                       Re_initializeDataObjectToSync(id_projet).then(function(){
                              console.info($scope.dataToSync.length);
                             
                              for (var vvv= 0; vvv < $scope.dataToSync.length ; vvv++) {
                                      chainage = chainage.then(function() {
                                            return  Re_dynamicDataToServer($scope.Recount);
                                      });
                               }

                              chainage.finally(function(){
